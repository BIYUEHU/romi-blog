          ...
          Some(ResNewsData {
            ...
            imgs: news
              ...
              .map(|imgs| {
                imgs
                  .split(',')
                  .filter_map(|s| s.is_empty().then_some(None).unwrap_or(Some(s.to_string())))
                  .collect()
              })
              .unwrap_or_default(),
          })
...
async fn create(
  ...
  let news = romi_news::ActiveModel {
    ...
    private: ActiveValue::set(if news.private { 1 } else { 0 }.to_string()),
  ...
  l_info!(logger, "Created news {} by admin {} ({})", news.nid, admin_user.id, admin_user.username);
  api_ok(())
}
...
async fn update(
    ...
    .with_context(|| format!("Failed to fetch news {}", id))?
  {
    Some(model) => {
      ...
      active_model.private = ActiveValue::set(if news.private { 1 } else { 0 }.to_string());
      active_model.imgs = ActiveValue::set(Some(news.imgs.join(",")));

      active_model.update(conn).await.with_context(|| format!("Failed to update news {}", id))?
    }
    None => {
      l_warn!(logger, "News {} not found", id);
      return Err(ApiError::not_found("News not found"));
...
async fn like(
    ...
    .with_context(|| format!("Failed to fetch news {} for like", id))?
  {
    Some(model) => {
      ...
      active_model.update(conn).await.with_context(|| format!("Failed to like news {}", id))?;
...
async fn view(
    ...
    .with_context(|| format!("Failed to fetch news {} for view", id))?
  {
    Some(model) => {
      ...
      active_model.update(conn).await.with_context(|| format!("Failed to view news {}", id))?;
