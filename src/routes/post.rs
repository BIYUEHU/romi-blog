...
async fn get_post_metas(
  ...
  let metas =
    romi_metas::Entity::find().filter(romi_metas::Column::Mid.is_in(meta_ids)).all(conn).await?;
    ...
    posts
      ...
      .filter_map(|data| {
        if data.hide.ne(&1.to_string()) {
          let (tags, categories) = post_metas.get(&data.pid).cloned().unwrap_or((vec![], vec![]));
          ...
          Some(ResPostData {
            ...
            password: password.map(|p| {
              access.level.eq(&AccessLevel::Admin).then_some(p).unwrap_or("password".into())
            }),
    ...
    .with_context(|| format!("Failed to fetch post {}", id))?
  {
    Some(model) => {
      ...
      api_ok(ResPostSingleData {
        ...
        password: password
          .map(|p| access.level.eq(&AccessLevel::Admin).then_some(p).unwrap_or("password".into())),
...
async fn handle_metas(
  ...
  let new_names: Vec<_> = names.into_iter().filter(|name| !existing_names.contains(name)).collect();
...
async fn update(
  ...
  let txn =
    conn.begin().await.with_context(|| format!("Failed to begin transaction for post {}", id))?;
    ...
    .with_context(|| format!("Failed to fetch post {} for update", id))?
  {
    Some(model) => {
      ...
      active_model.update(&txn).await.with_context(|| format!("Failed to update post {}", id))?;
    }
    None => {
      l_warn!(logger, "Post {} not found", id);
...
async fn like(
    ...
    .with_context(|| format!("Failed to fetch post {} for like", id))?
  {
    Some(model) => {
      ...
      active_model.update(conn).await.with_context(|| format!("Failed to like post {}", id))?;
...
async fn view(
    ...
    .with_context(|| format!("Failed to fetch post {} for view", id))?
  {
    Some(model) => {
      ...
      active_model.update(conn).await.with_context(|| format!("Failed to view post {}", id))?;
