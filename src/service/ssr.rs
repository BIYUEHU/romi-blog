...
impl ServerSideRender {
  pub fn new(file: String, port: u16, logger: Logger) -> Self {
    ...
    l_info!(logger, "ServerSideRender launched ➜ http://127.0.0.1:{}", port);
  ...
  pub async fn render(&self, url: &str) -> Result<ServerSideRenderResponse> {
    if self.process.is_none() {
      return Ok(ServerSideRenderResponse {
    ...
    let response =
      self.client.get(&request_url).send().await.context("Failed to connect to Node.js server")?;
    ...
    Ok(ServerSideRenderResponse { status, headers, body })
  }
...
pub struct ServerSideRenderResponse {
  pub status: StatusCode,
  pub headers: Vec<(String, String)>,
  pub body: Vec<u8>,
}

impl IntoResponse for ServerSideRenderResponse {
