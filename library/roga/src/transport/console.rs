...
impl<'a> Transport for ConsoleTransport<'a> {
  fn handle(&self, data: &LogData) {
    ...
    let pid =
      if self.use_color { data.pid.to_string().bold().to_string() } else { data.pid.to_string() };
    let labels = data
      .label
      .iter()
      .map(|name| self.label_template.replace("{name}", &name.color(self.label_color).to_string()))
      .collect::<Vec<_>>()
      .join(" ");
