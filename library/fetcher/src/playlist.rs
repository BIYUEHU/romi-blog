...
pub async fn fetch_playlist(
    ...
    let playlist_url = if csrf_token.is_empty() {
        ...
        format!("https://music.163.com/api/v3/playlist/detail?id={}", playlist_id)
    } else {
        format!(
    ...
    let OriginPlaylistParentData { playlist: OriginPlaylistData { track_ids } } =
    ...
    for (index, track) in track_ids.iter().enumerate() {
        let logger = logger.clone().with_label(format!("{}/{}", index + 1, total));

        let OriginTrackData { id } = track;

        let parse_song_info = async || -> Option<SongInfo> {
            let OriginSongsData { songs } = match client
                .get(&format!("https://music.163.com/api/song/detail/?id={}&ids=[{}]", id, id))
                .send()
                .await
            {
            ...
            let OriginSongData { name, artists, album: OriginAlbumData { pic_url: cover } } =
                match songs.first() {
                    Some(data) => data,
                    _ => {
            ...
            let artist = artists.first().map(|artist| artist.name.clone()).unwrap_or_default();
            let url = format!("http://music.163.com/song/media/outer/url?id={}.mp3", id);

            let lrc = match client
                .get(&format!("https://music.163.com/api/song/lyric?id={}&lv=1&kv=1&tv=-1", id))
                .send()
                .await
            {
                Ok(resp) => resp
                    .json::<OriginLrcData>()
                    .await
                    .map(|OriginLrcData { lrc: OriginLryicData { lyric } }| lyric)
                    .unwrap_or_default(),
                Err(_) => "".to_string(),
            };

            l_info!(logger, "Fetched {} - {} ({})", name, artist, id);
            Some(SongInfo { name: name.clone(), artist, url, cover: cover.clone(), lrc })
        };
